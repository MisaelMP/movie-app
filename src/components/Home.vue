...
const loadMovies = (value, page) => store.dispatch('LOAD_MOVIES_PAGINATION', {value, page});
const getAllMovies = computed(() => store.getters.getMoviesPagination.Search)
const getTotalMovies = computed(() => store.getters.getMoviesPagination.totalResults)
const getCurrentPage = computed(() => store.getters.getCurrentPage)
const setCurrentPage = () => store.commit('SET_CURRENT_PAGE', getCurrentPage.value + 1)
const setPreviousPage = () => store.commit('SET_CURRENT_PAGE', getCurrentPage.value - 1)
